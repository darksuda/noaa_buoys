...
for buoy in buoys:
    ...
    data_df_m = data_df.groupby(['time']).mean().reset_index()
    data_df_d = data_df.groupby(['time']).std().reset_index()
    data_df_m['std'] = data_df_d['sst']

    #data_df_m.to_csv('SST/HIST/CSV/hist_sst_' + buoy + '.csv', index=False)
    data_df_m.to_csv('daaata.csv', index=False)
